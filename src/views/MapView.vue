...
const files = ref<{ name: string; points: { lat: number; lng: number }[] }[]>(
  [],
);
...
function parseGpxToPoints(gpx: string) {
  ...
  for (let i = 0; i < trkpts.length; i++) {
    const element = trkpts[i] as Element;
    const lat = parseFloat(element.getAttribute("lat") || "0");
    const lng = parseFloat(element.getAttribute("lon") || "0");
...
function updateMap() {
  ...
  selectedFiles.value.forEach((file) => {
    ...
    if (fileData) {
      const route = L.polyline(fileData.points, {
        color: "black",
        weight: 3,
        opacity: 0.5,
      }).addTo(map);
